    ...
    public class MoviesController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<MovieDTO>>> Filter([FromQuery] FilterMoviesDTO filterMoviesDTO)
        {
            ...
            if (!string.IsNullOrWhiteSpace(filterMoviesDTO.OrderingField))
            {
                ...
                {
                    moviesQueryable = moviesQueryable
                        .OrderBy($"{filterMoviesDTO.OrderingField} " +
                        $"{(filterMoviesDTO.AscendingOrder ? "ascending" : "descending")}");
                }
                catch
                {
                    //log this
            ...
            await HttpContext.InsertPaginationParameteresInResponse(moviesQueryable, 
        ...
        public async Task<ActionResult> Post([FromForm] MovieCreationDTO movieCreationDTO)
        {
            ...
            if (movieCreationDTO.Poster != null)
            {
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    movie.Poster = await fileStorageService.SaveFile(content, extension,
        ...
        public async Task<ActionResult> Put(int id, [FromForm] MovieCreationDTO movieCreationDTO)
        {
            ...
            if (movieCreationDTO.Poster != null)
            {
                using (var memoryStream = new MemoryStream())
                {
                    ...
                    var content = memoryStream.ToArray();
                    var extension = Path.GetExtension(movieCreationDTO.Poster.FileName);
                    movieDB.Poster = await fileStorageService.EditFile(content, extension,
                                                                      containerName, movieDB.Poster, movieCreationDTO.Poster.ContentType);
                }
            }
